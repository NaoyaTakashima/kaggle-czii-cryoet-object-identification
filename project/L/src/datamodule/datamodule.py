...
from monai.transforms import (AsDiscrete, Compose, EnsureChannelFirstd,
                              ...
                              RandAffined, RandZoomd, RandGaussianSmoothd, RandCropd,
                              RandWeightedCropd)
...
class CZIIDataModule(LightningDataModule):
    ...
    def train_dataloader(self):
        train_files = [
            {
            "image": np.load(os.path.join(self.cfg.dir.data_dir_x, id.split('-')[1], f"train_image_{id.split('-')[0]}.npy")),
            "label": np.load(os.path.join(self.cfg.dir.data_dir_y,  f"train_label_{id.split('-')[0]}.npy"))[0]
            }
