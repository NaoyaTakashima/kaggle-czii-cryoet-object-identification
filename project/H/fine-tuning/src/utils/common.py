...
def reconstruct_array_gpu(
    ...
    --------
    torch.Tensor
        再構築された配列 (torch.Tensor)
    """
    reconstructed = torch.zeros(original_shape, dtype=patches.dtype, device=patches.device)  # 初期化

    d, h, w = patches.shape[1:]  # パッチサイズ
